  ...
  public function control($data = [])
  {
    ...
    switch ($data['action']) {
      ...
      case 'update':
        ...
        break;
      case 'changepw':
        $_URI = 'https://chshuju.com/api/cloudop.asp';
        //修改密码
    ...
    if ($data['action'] == 'activate') {
      ...
      $saveres = Db::name('vps_chshuju')->insert($saveData);
      if (!$saveres) {
        $ret['status'] = 500;
        $ret['msg'] = '操作超时！';
      }
      $savevpsres = Db::name('vps')->insert(['user_id' => $data['user_id'], 'plug_id' => $data['plug_id'], 'plug_type' => 'vps_chshuju', 'plug_name' => 'chshuju', 'pro_id' => Db::name('vps_chshuju')->where('name = "' . $res['vpsname'] . '"')->find()['id'], 'create_time' => time(), 'update_time' => time()]);
      ...
      $ret['data'] = '';
      $up = Db::name('vps_chshuju')->where('name = "' . $data['data']['vpsname'] . '"')->update(['password' => $data['data']['password']]);
    } else if ($data['action'] == 'renew') {
      $ret['data'] = '';
      $up = Db::name('vps_chshuju')->where('name = "' . $data['data']['vpsname'] . '"')->update(['end_time' => $res['endtime']]);
