...
namespace addons\vps\chuangmeng\controller;
...
class chuangmeng extends Controller
{
  /**
   * 主控端接口
   * @param $data  所传参数
  ...
  public function control($data = [])
  {
    ...
    if ($data['action'] != 'manager') {
      ...
      $data['userstr'] = md5($config['password'] . '150cn.com');
    }
    ...
    if ($data['action'] == 'activate') {
      ...
      $saveres = Db::name('vps_chuangmeng')->insert($saveData);
      if (!$saveres) {
        $ret['status'] = 500;
        $ret['msg'] = '操作超时！';
      }
      $savevpsres = Db::name('vps')->insert(['user_id' => $data['user_id'], 'plug_id' => $data['plug_id'], 'plug_type' => 'vps_chuangmeng', 'plug_name' => 'chuangmeng', 'pro_id' => Db::name('vps_chuangmeng')->where('name = "' . $res['vpsname'] . '"')->find()['id'], 'create_time' => time(), 'update_time' => time()]);
      ...
      $ret['data'] = '';
      $up = Db::name('vps_chuangmeng')->where('name = "' . $data['data']['vpsname'] . '"')->update(['password' => $data['data']['password']]);
    } else if ($data['action'] == 'renew') {
      $ret['data'] = '';
      $up = Db::name('vps_chuangmeng')->where('name = "' . $data['data']['vpsname'] . '"')->update(['end_time' => $res['endtime']]);
