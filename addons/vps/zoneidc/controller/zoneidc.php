...
class zoneidc extends Controller
{
  ...
  public function control($data = [])
  {
    ...
    $_URI = 'http://api.ipcomserver.com/api/cloudapi.asp';
    ...
    switch ($data['action']) {
      ...
      case 'update':
        ...
        break;
      case 'changepw':
        $_URI = 'http://api.ipcomserver.com/api/cloudop.asp';
        //修改密码
        ...
        break;
      case 'manager':
        $post = '<form id="sub" action="http://api.ipcomserver.com/vpsadm/zndatalogin.asp" method="post"><input type="hidden" name="vpsname" value="' . $data['data']['vpsname'] . '"/><input type="hidden" name="vpspassword" value="' . $data['data']['password'] . '"/></form>';
    ...
    if ($data['action'] == 'activate') {
      ...
      $saveres = Db::name('vps_zoneidc')->insert($saveData);
      if (!$saveres) {
        $ret['status'] = 500;
        $ret['msg'] = '操作超时！';
      }
      $savevpsres = Db::name('vps')->insert(['user_id' => $data['user_id'], 'plug_id' => $data['plug_id'], 'plug_type' => 'vps_zoneidc', 'plug_name' => 'zoneidc', 'pro_id' => Db::name('vps_zoneidc')->where('name = "' . $res['vpsname'] . '"')->find()['id'], 'create_time' => time(), 'update_time' => time()]);
      ...
      $ret['data'] = '';
      $up = Db::name('vps_zoneidc')->where('name = "' . $data['data']['vpsname'] . '"')->update(['end_time' => $res['endtime']]);
