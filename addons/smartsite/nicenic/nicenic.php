...
class nicenic extends Addons
{
    ...
    public function uninstall()
    {
        ...
        if (Db::query('show tables like "gee_smartsite_nicenic"')) {
            $sql = "DROP TABLE gee_smartsite_nicenic";
            if (!mysqli_query($conn, $sql)) {
                $ret['status'] = 500;
        ...
        $smartsite->where('plug_name = "nicenic"')->delete();
        // return $ret;

        //先删除目录下的文件：
        $dir = './../addons/smartsite/nicenic';
        $this->deldir($dir);
        $dir = './../application/nicenic';
        $this->deldir($dir);
        return $ret;
    }
