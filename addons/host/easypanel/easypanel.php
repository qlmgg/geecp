 ...
 * User: jingmian
 */
namespace addons\host\easypanel;
use think\Addons;
use \think\Db;
use \think\Route;
...
class easypanel extends Addons
{
    public  $ret = [
        'status'=>200,
        'msg'=>'操作成功',
    ...
    public function uninstall()
    {
        ...
        if(Db::query('show tables like "gee_vps_easypanel"')){
            $sql = "DROP TABLE gee_vps_easypanel";
            if (!mysqli_query($conn, $sql)) {
                $ret['status'] = 500;
        ...
        $vps = new GeeVps();
        $vps->where('plug_name = "easypanel"')->delete();
        // return $ret;

        //先删除目录下的文件：
        $dir = './../addons/host/easypanel';
        $this->deldir($dir);
        $dir = './../application/easypanel';
        $this->deldir($dir);
        return $ret;
    }
