 ...
 * User: jingmian
 */
namespace addons\cdn\easypanelcdn\controller;

use addons\cdn\easypanelcdn\common\library\WhmException;
use addons\cdn\easypanelcdn\model\GeeCdnEasypanel;
...
class easypanelcdn extends Controller{
    ...
    //  dump($data);
      ...
      // dump($data);
        ...
        $port = $config['prot'];
        $skey = $config['skey'];
        $easypanel= new \addons\cdn\easypanelcdn\common\helper\easypanelcdn();
      ...
      try{
          switch($data['action']){
              ...
              case 'renew':
                  ...
                  $cdndata = $cdnmodel->find($data['id']);
                  $start = date("Y-m-d",$cdndata->getData('create_time'));
                  $end = date("Y-m-d",$cdndata['end_time']);
          ...
          if($whmException->type =="getVh"){
              $ret['status'] =404;
              $ret['type'] ="getVh";
              $ret['msg']=$params['username']."cdn站点名字不存在";
       ...
       //  var_dump($ret);die();
        ...
        try {
            ...
            if($data['action'] == 'activate'){
                ...
                $savevpsres = Db::name('cdn')->insert(
                    [
                        'user_id'=>$data['user_id'],
                        'status'=> "正常",
                        'plug_id'=>$data['plug_id'],
                        'plug_type'=>'cdn_easypanel',
                        'plug_name'=>'easypanelcdn',
                        'pro_id'=>$id,
                        'type'=>isset($data['host_type']) ? $data['host_type']:null,
                        'create_time'=>time(),
                        'update_time'=>time(),
              ...
              //  $up = Db::name('vps_zoneidc')->where('name = "'.$data['data']['vpsname'].'"')->update(['password'=>$data['data']['password']]);
                ...
                $cdndata->save(["end_time"=>strtotime("+".$params['month']." month",time()-3600)]);
                $easymodel->save(
                ...
                $ret['data'] = '';
                $up = Db::name('cdn_easypanel')->where('name = "'.$data['data']['hostname'].'"')->update(['end_time'=>$res['endtime']]);
            }
            Db::commit();
    ...
    public function updateList($data=[]){
      $v = Db::name('cdn');
      $cdn = Db::name('cdn_easypanel');
      $easymodel = new GeeCdnEasypanel();
      // $plug= $this->control();
      if($data['data']['user_id'] == '-1'){
        $vpsList = $cdn->select();
      } else {
        $vpsList = $cdn->where('user_id = '.$data['data']['user_id'])->select();
      }
      try{
