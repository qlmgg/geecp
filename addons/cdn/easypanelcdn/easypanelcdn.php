<?php
namespace addons\cdn\easypanelcdn;
/**
...
use app\index\model\GeeCdn; // 前台路由表
use app\manager\model\GeeAddons; // 插件表
use ZipArchive; // PHP自带zip解析
class easypanelcdn extends Addons
{
    public  $ret = [
        'status'=>200,
        'msg'=>'操作成功',
    ...
    public function uninstall()
    {
        ...
        if(Db::query('show tables like "gee_cdn_easypanelcdn"')){
            $sql = "DROP TABLE gee_cdn_easypanelcdn";
            if (!mysqli_query($conn, $sql)) {
                $ret['status'] = 500;
        ...
        $vps = new GeeCdn();
        $vps->where('plug_name = "easypanelcdn"')->delete();
        // return $ret;

        //先删除目录下的文件：
        $dir = './../addons/cdn/easypanelcdn';
        $this->deldir($dir);
        $dir = './../application/easypanelcdn';
        $this->deldir($dir);
        return $ret;
    }
