...
class Login extends Controller
{
    ...
    public function auth()
    {
        ...
        $userinfo = $user->where('`phone` = "' . $data['username'] . '" or `username` = "'.$data['username'].'" or email = "'.$data['username'].'"')->find();
        if (!$userinfo) {
            $ret['status'] = 422;
            $ret['msg'] = '手机号或密码错误！';
            return $ret;
        ...
        if (password_verify($data['password'], $userinfo['password'])) {
            ...
            $wxtid = $mm->where('mark = "loginRemind"')->find()['wx_temp_id'];
            $params = [
              'title' => ['value' => '账号登录提醒'],
              'loginTime' => ['value' => date('Y年m月d日 H:i:s',time())],
              'remark' => ['value' => '您的账号于'.date('Y年m月d日 H:i:s',time()).'登录，如不为本人操作请尽快修改密码！']
            ];
