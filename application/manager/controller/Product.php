...
use app\manager\model\GeeAddons; // 请求类
use app\manager\model\GeeLog; // 日志表
use app\manager\model\GeeMsgmodel; // 邮件模板表
use app\manager\model\GeeOsgroup; // 操作系统表
use app\manager\model\GeeOstype; // 操作系统版本表
...
use app\manager\model\GeeServerAdded; // 增值服务组表
use app\manager\model\GeeServerAddedItems; // 增值服务子项表
use app\manager\model\GeeUsergroup; // 用户组表
...
class Product extends Common
{
    ...
    {
        return $this->redirect('manager/Product/list');
    }
    function list() {
        ...
        foreach ($groupList as $k => $v) {
            $groupList[$k]['child'] = $product->where('group_id', $v['id'])->order('sort desc,id desc')->select();
        }
    ...
    public function add()
    {
        ...
        //代理列表
        $agent = new GeeUsergroup();
        $agentlist = $agent->where('is_agent = 1')->select();
        ...
        if ($_GET['id']) {
            ...
            $productInfo['agentprice'] = json_decode($productInfo['agentprice'], true); //代理json
            $productInfo['rebates'] = json_decode($productInfo['rebates'], true); //返点json
            $this->assign('updatelist', $updatelist);
            ...
            if ($productInfo['plug_config']) {
                ...
                if (!empty($nowPlugConfig)) {
                    foreach ($nowPlugConfig as $k => $v) {
                        ...
                        switch ($v['type']) {
                            case 'text':
                                $html .= '<div class="form-group"><label for="' . $k . '" class="col-sm-2 control-label">' . ($v['title'] ? $v['title'] : $k) . '</label><div class="col-2"><input type="' . $v['type'] . '" class="form-control form-control-sm" id="' . $k . '" name="' . $k . '" placeholder="' . ($v['place'] ? $v['place'] : '请输入' . ($v['title'] ? $v['title'] : $k)) . '" value="' . $nowConfig[$k] . '" autocomplete="off"></div></div>';
                                break;
                            case 'password':
                                $html .= '<div class="form-group"><label for="' . $k . '" class="col-sm-2 control-label">' . ($v['title'] ? $v['title'] : $k) . '</label><div class="col-2"><input type="' . $v['type'] . '" class="form-control form-control-sm" id="' . $k . '" name="' . $k . '" placeholder="' . ($v['place'] ? $v['place'] : '请输入' . ($v['title'] ? $v['title'] : $k)) . '" value="' . $nowConfig[$k] . '" autocomplete="off"></div></div>';
                                break;
                            case 'textarea':
                                $html .= '<div class="form-group"><label for="' . $k . '" class="col-sm-2 control-label">' . ($v['title'] ? $v['title'] : $k) . '</label><div class="col-2"><textarea name="' . $k . '" id="' . $k . '" class="form-control form-control-sm" cols="30" rows="10" placeholder="' . ($v['place'] ? $v['place'] : '请输入' . ($v['title'] ? $v['title'] : $k)) . '">' . $nowConfig[$k] . '</textarea></div></div>';
                                break;
                        }
                    }
                }
    ...
    public function changeClassStatus()
    {
        ...
        $_s = $data['status'] ? '1' : '0';
        // dump($_s);
        $wrres = $wr->where('name', 'like', '/' . $cinfo['name'] . '%')->update([
            'status' => $_s,
        ]);
        // dump($wrres);
        $res = $c->where('id = ' . $data['id'])->update([
            'status' => $_s,
        ]);
        // dump($wrres);
        ...
        if ($res || $wrres) {
            $saveInfo = [
                'content' => $this->_adminInfo['name'] . ' ' . ($data['status'] ? '启用' : '禁用') . '了产品分类ID ' . $data['id'],
                'ip' => get_ip(),
            ];
            $logres = $log->save($saveInfo);
        } else {
    ...
    public function added()
    {
        ...
        $groupList = $group->order('id desc')->select();
        foreach ($groupList as $k => $v) {
            $groupList[$k]['child'] = $itemList = $item->where('group_id', $v['id'])->order('id desc')->select();
        }
        // dump($groupList);
    ...
    public function os()
    {
        ...
        $groupList = $group->order('id desc')->select();
        foreach ($groupList as $k => $v) {
            $groupList[$k]['child'] = $item->where('group_id', $v['id'])->order('id desc')->select();
        }
        $this->assign('list', $itemList);
        $this->assign('grouplist', $groupList);
