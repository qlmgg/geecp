...
use app\manager\model\GeeVhostGroup; // 虚拟主机配置分组表
use app\manager\model\GeeVhostConfig; // 虚拟主机配置表
use app\manager\model\GeeVhost; // 用户虚拟主机表
use addons\vhost\vhost; // 调用vhost插件

class Vhosts extends Common
{
    ...
    public function addvhost(){
        ...
        return $this->fetch('Vhost/addvhost');
    }

    public function addvhostAuth(){
    	$vhost = new GeeVhost();
		  $log = new GeeLog();
    	$data = $_POST;
    	$ret = [
