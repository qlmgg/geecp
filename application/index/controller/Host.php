...
class Host extends Common
{
  ...
  public function add()
  {
    ...
    if ($_GET['id']) {
      ...
      $items = !empty($item['update_list']) && $item['update_list'] != 'null'  ? explode(',', $item['update_list']) : [];
  ...
  public function addAuth()
  {
    ...
    $proinfo = $pro->where('id = '.$product_id)->find();
    $ainfo = $a->where('id = '.$proinfo['plug'])->find();
    if($ainfo['name'] == 'direct'){
      $hasDomain = Db::name('host_direct')->where('attach = "'.$data['domain'].'"')->find();
      if(!!$hasDomain){
        $ret['status'] = 422;
        $ret['msg'] = '该域名已存在';
        return json_encode($ret);
