...
class Billing extends Common
{
  ...
  public function order()
  {
    ...
    $list = $o->where('user_id = ' . $uinfo['id'] . ' or manager_id = ' . $uinfo['id'] . $w)->order('id desc')->paginate(10, false, ['query' => request()->param()]);
    $this->assign('list', $list);
  ...
  public function coupon()
  {
    ...
    $list = $c->where('user_id = ' . $uinfo['id'] . $w)->order('id desc')->paginate(10, false, ['query' => request()->param()]);
    $this->assign('list', $list);
    return $this->fetch('Billing/coupon');
  }
  ...
  public function rechargeauth()
  {
    ...
    $orderdata['balance'] = (float) session('_userInfo')['balance'] - (float) $data['money'];
    $orderdata['channel_type'] = $data['channel'];
    ...
    if($data['channel'] == 1){
      ...
      //微信
      $url = 'http://' . $_SERVER['HTTP_HOST'] . '/buy/wxpay?order='.$paypost['trade_no'];
