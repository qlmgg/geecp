...
use app\manager\model\GeeCloudserver; //产品分类表
use app\manager\model\GeeOsgroup; //产品组表
use app\manager\model\GeeOstype; //产品表
use app\manager\model\GeeProduct; // 操作系统组表
use app\manager\model\GeeProductClass; //操作系统版本表
use app\manager\model\GeeProductGroup; //插件表
...
class Cloudserver extends Common
{
    ...
    public function updatelist(){
      $cs = new GeeCloudserver();
      $plug = new \addons\cloudserver\cloudserver();
      $list = $cs->where('user_id = ' . session('_userInfo')['id'])->order('id desc')->select();
      foreach ($list as $k => $v) {
          //已到期
          if ($v['end_time'] <= time()) {
              $list[$k]['status'] = 0;
    ...
    public function create()
    {
        ...
        $plug = new \addons\cloudserver\cloudserver();
        //获取默认产品分类
        $grouplist = $group->where('class_id = 9')->order('sort desc')->select();
        //获取默认产品列表
        $prolist = $pro->where('group_id = ' . $grouplist[0]['id'])->order('sort desc')->select();
        //获取默认操作系统
    ...
    public function addAuth()
    {
        ...
        if ($data['vname']) {
            //续费操作
            $_data = $data;
            $data = $cs->where('name = "' . $_data['vname'] . '"')->find();
            $data['pay_length'] = $_data['pay_length'];
        ...
        if (!$data['type']) {
            ...
            $datas = [
                ...
                'net_out' => $data['mbps'],
                'net_in' => $data['mbps'],
                'end_time' => strtotime(date("Y-m-d H:i:s", strtotime("+" . ($data['pay_length']) . " month", $data['end_time']))),
                'ip_num' => $proConfig['ip_num'],
                'flow_limit' => $proConfig['flow_limit'],
                'nat_acl_limit' => $proConfig['nat_acl_limit'],
                'nat_web_limit' => $proConfig['nat_web_limit'],
    ...
    public function getProlist()
    {
        ...
        if ($_POST['type'] == 'update') {
            ...
            $proList = $pro->field('plug_config,update_list', true)->where('group_id = ' . $id)->order('sort desc')->select();
        }
        $ret['data'] = $proList;
        return json_encode($ret);
    }
