...
/**
 * 获取短信验证码
 * @param {Object} 触发按钮 JQ.fn
 * @param {String} 请求地址
 * @param {Object} 手机号文本框对象
 * @param {Number} 短信验证码读秒时间
 */
const _sendCode = {
  init: function (t, uri, phone, n) {
    const pval = phone.val();
    if (pval) {
      t.prop('disabled', true);
    }
    if (pval && (/^(1[3-9][0-9])\d{8}$/.test(pval))) {
      ajax(uri, {
        ...n,
        phone: pval,
      }, 'post').then(d => {
        if (d.status == 200) {
          _sendCode.times(t, 60);
        } else {
          alert(d.msg)
  ...
  times: function (t, n = 60) {
    ...
    if (time == 0) {
      ...
      if (t.next().length) {
        ...
        t.addClass('d-none');
        t.after(
          `<button type="button" class="btn btn-link bg-white border w-100 h-100 h-border-lightgray" disabled="disabled">${time}s</button>`
        );
      }
      time--;
      setTimeout(function () {
        _sendCode.times(t, time);
      }, 1000)
    }
  }
}
