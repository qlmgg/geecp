...
use app\admin\model\GeeDomainContact; //用户表
// use app\index\model\GeeDeal; //收支明细表
use app\admin\model\GeeDomainPrice; //交易记录表
use app\admin\model\GeeProduct; //订单表
use app\admin\model\GeeProductClass; //订单表
use app\admin\model\GeeProductGroup; //产品分类表
                ...
                if ($configs['_create_putData']) {
                    if ($configs['_create_putData']['class'] == 'domain') {
                        ...
                        if ($isfailed) {
                            // dump($uinfo['balance']);
                            // dump($info['money']);
                            $userup = db('user')->where('id = ' . $dinfo['user_id'])->update(['balance' => ((double) $uinfo['balance'] - (double) $dinfo['money']) + (double) $dinfo['money']]);
