...
use addons\cdn\easypanel\controller\easypanel;
...
class Cdn extends Common
{
    ...
    {
        $cdn = new GeeCdn();
        $list = $cdn->where('user_id = '.session('_userInfo')['id'])->order('id desc')->paginate(10);
        foreach($list as $k=>$v){
    ...
    public function add()
    {
        ...
        if($_GET['id']){
            ...
            $proItems = $pro->where('type=12')->select();
        }
        $groups = [];
        $groupList = [];
        // dump($proItems);
    ...
    public function addAuth(){
        ...
        //套餐类型
        $cdn_type  = json_decode($item['plug_config'],true)['type'];
        //产品类型
