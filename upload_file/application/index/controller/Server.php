...
use app\manager\model\GeeOsgroup; // 前置操作
use app\manager\model\GeeOstype; //产品分类表
...
class Server extends Common
{
    ...
    public function updatelist(){
      ...
      $osgroup = new GeeOsgroup();
      $ostype = new GeeOstype();
      $serverlist = $server->where('user_id = ' . session('_userInfo')['id'])->order('id desc')->select();
      foreach($serverlist as $k=>$v){
        if($v['end_time'] <= time()){
          $server->where('id = '.$v['id'])->update(['status'=>1]);
        }
    ...
    public function add()
    {
        ...
        // dump($_GET['type']);
        if ($_GET['type'] != 'renew') {
            $proItems = $pro->where('type=8')->order('sort desc')->select();
            $groups = [];
            $groupList = [];
            // dump($proItems);
