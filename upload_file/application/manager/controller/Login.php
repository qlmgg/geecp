...
use app\manager\model\GeeAdminuser;
use app\manager\model\GeeLog; // 日志表
use app\manager\model\GeeStaffgroup; // 员工表
use app\manager\model\GeeWebbasic; // 员工组表
...
class Login extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_COOKIE['token']) && !empty($_COOKIE['token']) && jwt_decode($_COOKIE['token'])) {
            return $this->redirect('manager/Index/index');
        }
