...
instance1.defaults.baseURL = apiuri;
instance1.defaults.headers.common['Authorization'] = Cookies.get('token') || '';
instance1.defaults.params = {
  _: Math.round(new Date() / 1000)
}
instance1.defaults.timeout = 6000 * 60 * 5;
...
function ajax(url, params, type = 'get', files) {
  ...
  if (typeof url == 'object') {
    instance1.defaults.baseURL = url[0];
  } else {
    instance1.defaults.baseURL = apiuri;
  }
  if (type !== 'post') {
  ...
  return instance1(url, config).then(response => {
...
//全选&&反选
$('.table #ckallhead,.table + div #ckallfoot').click(function () {
  if ($(this).is(':checked')) {
    $(this).parents('.table-body').find('tbody input[type="checkbox"]').prop('checked', true);
    $('.table #ckallhead,.table + div #ckallfoot').prop('checked', true)
  } else {
    $(this).parents('.table-body').find('tbody input[type="checkbox"]').prop('checked', false);
    $('.table #ckallhead,.table + div #ckallfoot').prop('checked', false)
  }
});
$('.table input[type="checkbox"],.table + div input[type="checkbox"]').click(function () {
  if (!$(this).is(':checked')) {
    $('.table #ckallhead,.table + div #ckallfoot').prop('checked', false)
  } else {
    if ($(this).parents('.table-body').find('tbody input[type="checkbox"]').length == $(this).parents('.table-body').find('tbody input[type="checkbox"]:checked').length) {
      $('.table #ckallhead,.table + div #ckallfoot').prop('checked', true)
    }
  }
...
//设置广告图片
let setAd = ()=>{
  ajax('https://ad.geecp.com/manager/v2/api/getAdInfo', {}, 'get').then(e => {
    console.log(e)
    if(!!e.ad_install_ready_img){
      $('.ad_install_ready_img').prop('href',e.ad_install_ready_img.url)
      $('.ad_install_ready_img img').prop('src',e.ad_install_ready_img.src)
    } else {
      $('.ad_install_ready_img').remove()
    }
    if(e.ad_install_finish_img.length){
      e.ad_install_finish_img.map((v,i)=>{
        $('.ad_install_finish_img a').eq(i).prop('href',v.url)
        $('.ad_install_finish_img img').eq(i).prop('src',v.src)
      })
    } else {
      $('.ad_install_ready_img').remove()
    }
    if(e.ad_manager_login_img.length){
      e.ad_manager_login_img.map((v,i)=>{
        $('.ad_manager_login_img a').eq(i).prop('href',v.url)
        $('.ad_manager_login_img img').eq(i).prop('src',v.src)
      })
    } else {
      $('.ad_manager_login_img').remove()
    }
    if(e.ad_manager_home_img.length){
      e.ad_manager_home_img.map((v,i)=>{
        $('.ad_manager_home_img a').eq(i).prop('href',v.url)
        $('.ad_manager_home_img img').eq(i).prop('src',v.src)
      })
    } else {
      $('.ad_manager_home_img').remove()
    }
  });
}
setAd();